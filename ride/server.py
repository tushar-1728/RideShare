...
def addRide():
    ...
    try:
        ...
        if requests.get("http://34.226.90.178:8080/api/v1/db/read", params={"COMMAND":"EXISTS", "FIELD":"username", "VALUE":username, "DB":"Users"}).json()["count"] == 0:
            print(1)
            return make_response('',400)
...
def Join_ride(rideid):
    ...
    if requests.get("http://127.0.0.1:5000/api/v1/db/read", params={"COMMAND":"EXISTS", "FIELD":"_id", "VALUE":rideid, "DB":"Rides"}).json()["count"] == 0 or requests.get("http://34.226.90.178:8080/api/v1/db/read", params={"COMMAND":"EXISTS", "FIELD":"username", "VALUE":username, "DB":"Users"}).json()["count"] == 0:
        return make_response('',400)
