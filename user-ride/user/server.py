...
def read_all():
	requests.post("http://34.238.33.155:80/api/v1/db/write", data=json.dumps({"ORIGIN":"USER", "COMMAND":"ADD_REQUEST_COUNT"}))
	msg = requests.get("http://34.238.33.155:80/api/v1/db/read", params={"ORIGIN":"USER", "COMMAND":"READ_ALL", "DB":"Users"})
...
def delete_all():
	# add_request_count()
	requests.post("http://34.238.33.155:80/api/v1/db/write", data=json.dumps({"ORIGIN":"USER", "COMMAND":"DELETE_ALL"}))
	return make_response('', 200)

#api 12
@app.route('/api/v1/_count', methods=['GET'])
def count_requests():
	msg = requests.get("http://34.238.33.155:80/api/v1/db/read", params={"ORIGIN":"USER", "COMMAND":"READ_REQUEST_COUNT"})
...
def reset_request_count():
	requests.post("http://34.238.33.155:80/api/v1/db/write", data=json.dumps({"ORIGIN":"USER", "COMMAND":"RESET_REQUEST_COUNT"}))
	return make_response('', 200)
