...
def dbState(collection):
	try:
		myclient = pymongo.MongoClient('mongodb://users_mongodb:27017/')
	except:
		return False,False
...
def adduser():
	requests.post("http://localhost:5000/api/v1/db/write", data=json.dumps({"COMMAND":"ADD_REQUEST_COUNT"}))
...
def db_read():
	...
	if request.args.get('COMMAND') == "EXISTS":
		collection = dbState(request.args.get("DB"))
...
def count_requests():
	msg = requests.get("http://localhost:5000/api/v1/db/read", params={"COMMAND":"READ_REQUEST_COUNT"})
...
def reset_request_count():
	requests.post("http://localhost:5000/api/v1/db/write", data=json.dumps({"COMMAND":"RESET_REQUEST_COUNT"}))
	return make_response('', 200)
...
if __name__ == '__main__':
	client = pymongo.MongoClient('mongodb://users_mongodb:27017/')
